...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        optimal_action = max(legal_actions, key=lambda action: self.getQValue(state, action))
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        """
        "*** YOUR CODE HERE ***"
        updated_sample = reward + (self.discount * self.getValue(nextState))
        current_q_value = self.getQValue(state, action)
        self.qValue[(state, action)] = ((1 - self.alpha) * current_q_value) + (self.alpha * updated_sample)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        q_value = sum(self.weights[feat] * val for feat, val in extracted_features.items())
        return q_value
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        for feat, val in extracted_features.items():
            self.weights[feat] += self.alpha * correction * val
